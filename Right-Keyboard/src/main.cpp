...
void checkPriorityKeys() {
    ...
    if (!digitalRead(COL_2) && !digitalRead(COL_3) && !guiPressed) {
        ...
        guiPressed = true;
    } else if ((digitalRead(COL_2) || digitalRead(COL_3)) && guiPressed) {
    ...
    if (!digitalRead(COL_6) && !layer1Pressed && !altPressed) {
        bleKeyboard.press(KEY_LEFT_ALT);
        altPressed = true;
    } else if (layer1Pressed || (digitalRead(COL_6) && altPressed)) {
    ...
    if (bleKeyboard.isConnected()) {
        ...
        for (int i = 0; i < rowsCount; i++) {
            ...
            for (int j = 0; j < colCount; j++) {
                ...
                // Already checked
                if (!layer1Pressed && !layer2Pressed && specialKeys.count(keys[i][j])) continue;
                if (layer1Pressed && !layer2Pressed && specialKeys.count(layer1[i][j])) continue;
                if (!layer1Pressed && layer2Pressed && specialKeys.count(layer2[i][j])) continue;
